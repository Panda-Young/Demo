...
{
    if (version == NULL) {
        LOG_MSG_C(LOG_ERROR, "version is NULL");
...
void *algo_init()
{
    ...
    if (algo_handle == NULL) {
        LOG_MSG_C(LOG_ERROR, "allocate for algo_handle_t failed");
        return NULL;
    }
    memset(algo_handle, 0, sizeof(algo_handle_t));
    LOG_MSG_C(LOG_INFO, "algo_init OK");
    return algo_handle;
}
...
{
    if (!algo_handle) {
        LOG_MSG_C(LOG_ERROR, "algo_handle is NULL");
        return;
    }
    ...
    free(algo_handle);
    algo_handle = NULL;
    LOG_MSG_C(LOG_INFO, "algo_deinit OK");
}
...
int algo_set_param(void *algo_handle, algo_param_t cmd, void *param, int param_size)
{
    ...
    if (param == NULL) {
        LOG_MSG_C(LOG_ERROR, "param is NULL");
    ...
    case ALGO_PARAM1: {
        if (param_size != sizeof(char)) {
            sprintf(log_temp_buf, "Received param size: %d Bytes is not correct. Expected size is %zu", param_size, sizeof(char));
            LOG_MSG_C(LOG_ERROR, log_temp_buf);
            return E_PARAM_SIZE_INVALID;
        }
        algo_handle_ptr->param1 = *(char *)param;
        sprintf(log_temp_buf, "set param1: %c", algo_handle_ptr->param1);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM2: {
        if (param_size != sizeof(float)) {
            sprintf(log_temp_buf, "Received param size: %d Bytes is not correct. Expected size is %zu", param_size, sizeof(float));
            LOG_MSG_C(LOG_ERROR, log_temp_buf);
            return E_PARAM_SIZE_INVALID;
        }
        algo_handle_ptr->param2 = *(float *)param;
        sprintf(log_temp_buf, "set param2: %.1f", algo_handle_ptr->param2);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM3: {
        if (param_size > MAX_BUF_SIZE) {
            sprintf(log_temp_buf, "Received param size: %d Bytes is too large. Max size is %u", param_size, MAX_BUF_SIZE);
            LOG_MSG_C(LOG_ERROR, log_temp_buf);
            return E_PARAM_SIZE_INVALID;
        }
        memset(algo_handle_ptr->param3, 0, MAX_BUF_SIZE);
        memcpy(algo_handle_ptr->param3, param, param_size);
        sprintf(log_temp_buf, "set param3: %s", algo_handle_ptr->param3);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM4: {
        ...
        if (algo_handle_ptr->param4 == NULL) {
            sprintf(log_temp_buf, "allocate %d Bytes for param4 failed", param_size);
            LOG_MSG_C(LOG_ERROR, log_temp_buf);
    ...
    default:
        sprintf(log_temp_buf, "cmd %d is invalid", cmd);
        LOG_MSG_C(LOG_ERROR, log_temp_buf);
...
int algo_get_param(void *algo_handle, algo_param_t cmd, void *param, int param_size)
{
    ...
    case ALGO_PARAM1: {
        if (param_size != sizeof(char)) {
            LOG_MSG_C(LOG_ERROR, "param_size is not correct");
            return E_PARAM_SIZE_INVALID;
        }
        *(char *)param = algo_handle_ptr->param1;
        sprintf(log_temp_buf, "get param1: %c", algo_handle_ptr->param1);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM2: {
        ...
        *(float *)param = algo_handle_ptr->param2;
        sprintf(log_temp_buf, "get param2: %.1f", algo_handle_ptr->param2);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM3: {
        if (param_size > MAX_BUF_SIZE) {
            LOG_MSG_C(LOG_ERROR, "param_size is too large");
            return E_PARAM_SIZE_INVALID;
        }
        memcpy(param, algo_handle_ptr->param3, param_size);
        sprintf(log_temp_buf, "get param3: %s", algo_handle_ptr->param3);
        LOG_MSG_C(LOG_INFO, log_temp_buf);
        break;
    }
    case ALGO_PARAM4: {
        if (param_size > MAX_BUF_SIZE) {
            sprintf(log_temp_buf, "param_size %d is too large than MAX_BUF_SIZE %d", param_size, MAX_BUF_SIZE);
            LOG_MSG_C(LOG_ERROR, log_temp_buf);
            return E_PARAM_SIZE_INVALID;
        }
    ...
    default:
        LOG_MSG_C(LOG_ERROR, "cmd is invalid");
        return E_PARAM_OUT_OF_RANGE;
    }
    return E_OK;
}
...
int algo_process(void *algo_handle, float *input, float *output, int block_size)
{
    ...
    if (input == NULL) {
        LOG_MSG_C(LOG_ERROR, "input is NULL");
        return E_PARAM_BUFFER_NULL;
    }
    if (output == NULL) {
        LOG_MSG_C(LOG_ERROR, "output is NULL");
        return E_PARAM_BUFFER_NULL;
    }
    if (block_size <= 0) {
        LOG_MSG_C(LOG_ERROR, "block_size is not correct");
        return E_PARAM_SIZE_INVALID;
    }
    p_algo_handle_t algo_handle_ptr = (p_algo_handle_t)algo_handle;
